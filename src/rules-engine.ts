...
class StateCache {
  ...
  private getKey(tokenId: string, conversationId: string, accountId?: string): string {
    return `${tokenId}:${conversationId}:${accountId || ''}`;
  }

  get(tokenId: string, conversationId: string, accountId?: string): ConversationState | null {
  ...
  set(tokenId: string, conversationId: string, state: ConversationState, accountId?: string): void {
    const key = this.getKey(tokenId, conversationId, accountId);
  ...
  markDirty(tokenId: string, conversationId: string, accountId?: string): void {
    const key = this.getKey(tokenId, conversationId, accountId);
    const cached = this.cache.get(key);
    if (cached) {
      cached.dirty = true;
...
class RegexCache {
  ...
  get(pattern: string, flags: string = 'gi'): RegExp {
...
class ThresholdTracker {
  private trackers = new Map<string, Map<string, ThresholdEntry>>();

  private getKey(tokenId: string, conversationId: string, accountId?: string): string {
    return `${tokenId}:${conversationId}:${accountId || ''}`;
  }

  check(rule: Rule, tokenId: string, conversationId: string, accountId?: string): boolean {
    if (!rule.threshold || !rule.window) return true;
...
export class RulesEngine {
  ...
  private thresholdTracker = new ThresholdTracker();
   ...
   */
  private getRulePriority(rule: Rule): number {
  ...
  async evaluate(ctx: EvaluationContext): Promise<EvaluationResult[]> {
    ...
    for (const rule of eligibleRules) {
      ...
      if (rule.content && rule.content.length > 0) {
        ...
        if (!allKeywordsFound) continue; // Early exit
        matched = true;
      ...
      if (rule.pcre && rule.pcre.length > 0) {
        ...
        if (!allPatternsMatch) continue; // Early exit
        matched = true;
      }
   ...
   */
  private preFilterRules(rules: Rule[], state: ConversationState): Rule[] {
    return rules.filter(rule => {
      if (!rule.flags?.check || rule.flags.check.length === 0) return true;
  ...
  getStats() {
    return {
      ...
      cacheSize: (this.stateCache as any).cache.size,
      regexCacheSize: (this.regexCache as any).cache.size
    };
  }
